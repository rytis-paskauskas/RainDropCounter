#! /usr/bin/env python
# Time-stamp: <2024-09-09 15:01:37 rytis>
# Demonstrates building a single augmentation file from multiple recordings
...
# It is possible to join records into augmented files in multiple ways.
# Here we will collect all the generators and inputs in a list for a single augmented file output.
gen = []
inp = []
...
for i, dsid in enumerate([1, 3]):
    ...
    # We will customize the augmentation (random sampling) parameters
    # separately for each recording. For that purpose we perform the
    # parameter-free encoding first, and use the quantiles of delta t,
    # delta a to set the generation min and max.

    # The quantiles used for the paper are 0.75 for the minimum and
    # 0.99 for the maximum. In retrospect, both boundaries are too
    # extreme, especially the lower one. Something like [0.5, 0.9] would probably have been OK too.
    sig, r = read(inp_wav)
    encoded = encoder(sig)
    ...
    # We wish to combine all inputs into a single augmented output:
    gen.append(AugmentationGenerator(target_freq, lims_x, lims_y, num_augment))
    inp.append(inp_wav)

# The moment of truth, let's generate a single augmented file
aug_path = f'{aug_basepath}/aggregate.tfrec'
